...
FWTS is executed in SBBR mode using --sbbr together with UEFI variable stress options. In the default flow, the script runs FWTS with a fixed, explicit list of SBBR-relevant tests and appends all console output to a single log file.

### Command characteristics
	Runs FWTS in SBBR profile mode: --sbbr
	Enables repeated UEFI variable operations:
	--uefi-set-var-multiple=1
	--uefi-get-mn-count-multiple=1
##### Results Path: `acs_results/fwts/FWTSResults.log`

## Test Modules
The table below lists the test modules for the SBBR.
<table>
