...
FWTS is executed as part of compliance on the SystemReady band. When FWTS is invoked from the SystemReady band flow, the following command is executed.

### Command characteristics
**`fwts -r stdout -q --uefi-set-var-multiple=1 --uefi-get-mn-count-multiple=1 --sbbr aest cedt slit srat hmat pcct pdtt bgrt bert einj erst hest sdei nfit iort mpam ibft ras2`**

	sbbr: Runs all tests related to the SBBR test category
    q: Runs in quiet mode
    r stdout: Outputs to stdout format
	Enables repeated UEFI variable operations:
	--uefi-set-var-multiple=1
	--uefi-get-mn-count-multiple=1
    Executes the explicit SBBR-relevant test list: (esrt, uefibootpath, aest, cedt, slit, srat, hmat, pcct, pdtt, bgrt, bert, einj, erst, hest, sdei, nfit, iort, mpam, ibft, ras2)
