...
FWTS is executed as part of compliance on the SystemReady DT band. When FWTS is invoked from the DT band flow, the following command is executed.

### Command characteristics
**`fwts --ebbr smccc -r stdout`**

    ebbr: Runs all tests related to the EBBR test category.
    smccc: Runs the SMCCC FWTS tests.
    r stdout: Outputs results to stdout format.
