...
FWTS is executed in **EBBR mode** using `--ebbr`, which enables FWTS’s built-in EBBR test profile. In the default flow, the wrapper script runs FWTS with the EBBR profile and with additional platform-specific tests, while appending all console output to a single log file.

##### Results path: `acs_results/fwts/FWTSResults.log`

## Test Modules
The table below lists the **test modules** for the EBBR.
<table>
